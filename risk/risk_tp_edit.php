...
<?
	...
	$action = $_GET["action"];
	$risk_id= isset( $_GET["risk_id"] ) ? $_GET["risk_id"] : $_GET["risk_tp_id"];
	$tp_id= $_GET["tp_id"];
...
?>
	...
	<section id="content-wrapper">
		<h3>Edit or Create a Third Party Risk</h3>
...
<?
			if ($risk_item[risk_id]) {
				...
				foreach($pre_selected_tp_list as $pre_selected_tp_item) {
					array_push($pre_selected_items,$pre_selected_tp_item[risk_tp_join_tp_id]);
				}
			}
			list_drop_menu_tp($pre_selected_items,"tp_name");	
?>
		</select>
...
<?
		if ($risk_item[risk_id]) {
			...
			foreach($pre_selected_asset_list as $pre_selected_asset_item) {
				array_push($pre_selected_items,$pre_selected_asset_item[risk_tp_asset_join_asset_id]);
			}
		}
		list_drop_menu_asset($pre_selected_items,"asset_name");	
?>
						</select>
						<label for="name">Why are assets shared with this TP?</label>
						<span class="description">Most of the times elements of work are shared with other third parties. Some times this elements (assets) are of value for the organization and risk approaches are required to ensure appropiate controls are taken if necessary.</span>
<?
echo "						<textarea id=\"\" class=\"filter-text\" name=\"risk_tp_what\">$risk_item[risk_tp_what]</textarea>";
?>
						<label for="name">How it will be controled?</label>
						<span class="description">How will the access with this TP will be controled?</span>
<?
echo "						<textarea id=\"\" class=\"filter-text\" name=\"risk_tp_how\">$risk_item[risk_tp_how]</textarea>";
?>
...
<?
	global $risk_conf;
	if ($risk_conf['automatic_calculator_risk_score']) {
		echo "<span class=\"description\"><b>Warning!:</b> this score was automatically calculated<span>";
		echo "<br>";
		echo "<br>";
...
?>
		...
		<div class="controls-wrapper">
				    ...
				    <INPUT type="hidden" name="subsection" value="risk_tp_list">
